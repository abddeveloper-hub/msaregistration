...
function formatTeacherAddedDate(rawDate) {
    if (!rawDate) return '';
    let dateObj;
...
if (galleryAdminGrid) {
    onSnapshot(collection(db, "gallery"), (snap) => {
        ...
        snap.forEach(docSnap => {
            ...
            const addedDateStr = formatTeacherAddedDate(data.createdAt || data.timestamp);
...
if (achievementsAdminGrid) {
    onSnapshot(collection(db, "achievements"), (snap) => {
        ...
        snap.forEach(docSnap => {
            ...
            item.className = 'portal-card';
            item.style.padding = '0.75rem';
            const addedDateStr = formatTeacherAddedDate(data.createdAt || data.date);
